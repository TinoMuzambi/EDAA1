...
```{r}
coures.passed <- undergraduate %>%
  group_by(ID, Term) %>%
  summarise(NumCourses = n(), PassedCourses = sum(Num.Grade >= 50))

courses.passed.same.year <- coures.passed %>%
  filter(NumCourses == 2, PassedCourses == 2)

result <- courses.passed.same.year %>%
  group_by(Term) %>%
  summarise(TotalStudents = n(),
            Percentage = (n() / nrow(undergraduate)) * 100)
```
...
# Define a function to check if a course is in the 1st or 2nd year
is_first_or_second_year <- function(course_nbr) {
  return (grepl(".*2$", course_nbr))
}

undergrad.filter <- undergraduate[is_first_or_second_year(data$Catalog.Nbr), ]

passes.per.course <- undergraduate %>%
  filter(Num.Grade >= 50) %>%
  group_by(Catalog.Nbr) %>%
  count()
passes.per.course

num.stu.per.course <- undergraduate %>%
  group_by(Catalog.Nbr) %>%
  count()

pass.rate <- passes.per.course$n / num.stu.per.course$n
pass.rate
```
