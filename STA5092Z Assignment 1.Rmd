...
# Create vector of grade codes.
codes.to.replace <- c("DPR", "INC", "UP", "AB", "PA", "F", "LOA")

# Create vector of grades that were absent for the exam e.g 45A 
grades <- as.character(undergraduate$grade)
pattern <- "^\\d+[A]$" # Matches any number of digits followed by A

# Create vector of matching grades
a.grades <- grades[grepl(pattern, grades)]

# Replace all codes with NAs
undergraduate <- undergraduate %>%
  replace_with_na(replace = list(grade = codes.to.replace)) %>% 
  replace_with_na(replace = list(grade = a.grades))

# Convert grade to numerical type (assuming remaining values are numerical).
undergraduate <- undergraduate %>%
...
grade.box <- undergraduate %>%
  ...
  geom_boxplot() +
  labs(title = "Boxplot of Numerical Grade",
       x = "Numerical Grade (%)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
grade.box
...
grade.hist <- undergraduate %>%
  ggplot(aes(num.grade)) +
  geom_histogram() +
  labs(title = "Histogram of Numerical Grade Distribution",
       x = "Numerical Grade (%)",
       y = "Count") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
grade.density <- undergraduate %>% 
  ggplot(aes(num.grade)) +
  geom_density() +
  labs(title = "Density of Numerical Grade",
       x = "Numerical Grade (%)",
       y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
a1.a2.b1.joined$a.grade <- pmax(a1.a2.b1.joined$num.grade.x, a1.a2.b1.joined$num.grade.y)
...
quantile(a1.a2.b1.joined$a.grade[a1.a2.b1.joined$num.grade.y >= 50], 0.25)
```
...
b1.b2.joined <- merge(subset(b1.b2, course=="B1"), 
                   subset(b1.b2, course=="B2"),
                   by="id")
...
quantile(b1.b2.joined$num.grade.x[b1.b2.joined$num.grade.y >= 50], 0.25)
```
...
a1.b1 <- undergraduate %>% 
  filter(course %in% c("A1", "B1"))
a2.b2 <- undergraduate %>% 
  filter(course %in% c("A2", "B2"))

# Join data to link 1st and 2nd year grades by student.
merged.joined <- merge(a1.b1, 
                       a2.b2, 
                       by = "id", 
                       suffixes = c(".first", ".second"))

# Calculate the correlation.
correlation.first.second <- cor(merged.joined$num.grade.first, merged.joined$num.grade.second, use = "pairwise.complete.obs")
