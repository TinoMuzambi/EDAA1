...
# Compare pass rates between Covid year and non-Covid years
pass.rates.comparison <- undergraduate %>%
  group_by(Catalog.Nbr, Term) %>%
  summarise(Avg_Pass_Rate = mean(Num.Grade >= 50) * 100)

# Plot the comparison using ggplot2
ggplot(pass.rates.comparison, aes(x = as.factor(Term), y = Avg_Pass_Rate, fill = as.factor(Term == covid.year))) +
