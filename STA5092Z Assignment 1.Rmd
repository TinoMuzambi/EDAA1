...
# Create vectors of matching grades
failing.grades <- c("DPR", "INC", "AB", "F", "LOA")
passing.grades <- c("UP","PA")

# Replace codes with 0s and 50s
# Replace codes with appropriate values (consider using loop for efficiency)
undergraduate <- undergraduate %>%
  mutate(grade = ifelse(grade %in% failing.grades, 0,
                        ifelse(grade %in% passing.grades, 50, 
                               ifelse(grepl("^[0-9]+A$", grade),50,grade)))
  )
