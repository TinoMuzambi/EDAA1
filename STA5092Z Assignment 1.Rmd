...
undergraduate <- undergraduate %>%
            mutate(Num.Grade = ifelse(grepl("^[0-9]+$", Grade), as.integer(Grade), ifelse(Grade == "UP", 50, 0)))
str(undergraduate)
```
...
```{r}
num.stu.per.year <- undergraduate %>%
  group_by(Term) %>%
  count()

undergraduate %>%
  filter(
    ((Catalog.Nbr == "A1" & Num.Grade >= 50) & (Catalog.Nbr == "A2" & Num.Grade >= 50))) %>%
  group_by(Term) %>%
  summarise(n = n())

undergraduate %>%
  filter(
    ((Catalog.Nbr == "B1" & Num.Grade >= 50) & (Catalog.Nbr == "B2" & Num.Grade >= 50))) %>%
  group_by(Term) %>%
  summarise(n = n())
```
...
pass.rate <- passes.per.course / num.stu.per.course
pass.rate
```
